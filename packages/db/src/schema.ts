...
// the auth tables follow better-auth's core schema (user, session, account,
// verification). the drizzle adapter addresses tables by the key they are
// passed under and columns by their object key, so the camelCase keys below
// must match better-auth's field names; the db column names are ours
// (snake_case). ids are generated by postgres (advanced.database.generateId
// is false on the auth server), so all four tables default to gen_random_uuid.
// changing these tables? update migrations/ as well, the live db is migrated
// with plain sql, not with `drizzle-kit push`.
...
export const Account = pgTable(
  ...
  {
    id: uuid("id").notNull().primaryKey().defaultRandom(),
    // the user's id at the provider (google sub) or the user id for
    // credential-style providers
...
// magic link tokens, oauth state and similar short-lived values
...
export const PledgeFulfillment = pgTable(
  ...
  // schema-only changes like this one are applied with `pnpm db:push` (one-off
  // migrations that also touch data live in migrations/), and the push fails
